...
class Orders():
    ...
    def select_customer(self):
        def show_customer():
            ...
            for i in selection:
                ...
                self.contact = item[2]
                self.address = item[3]
                tk.Label(text=f'customer_id:    {self.customer_id}', name='customer customer_id').grid(row=5, column=0, padx=10, pady=0, sticky='w')
                tk.Label(text=f'name:                {self.name}', name='customer name').grid(row=6, column=0, padx=10, pady=0, sticky='w')
                tk.Label(text=f'contact:            {self.contact}', name='customer contact').grid(row=7, column=0, padx=10, pady=0, sticky='w')
                tk.Label(text=f'address:            {self.address}', name='customer address').grid(row=8, column=0, padx=10, pady=0, sticky='w')
    ...
    def edit_frame(self):
        ...
        if len(selection) != 1:
            ...
            # Настраиваем заголовкм
            tk.Label(text='Customer', font='bold', name='customer title').grid(row=4, column=0, padx=0, pady=0, sticky='w')
            tk.Label(text='Good', font='bold', name='good title').grid(row=4, column=1, padx=10, pady=0, sticky='w')
            tk.Label(text='Quantity', font='bold', name='quantity title').grid(row=4, column=2, padx=0, pady=0, sticky='w')

            self.customer_id_label = tk.Label(text=f'customer_id:    {self.customer_id}', name='customer customer_id')
            self.customer_id_label.grid(row=5, column=0,padx=10, pady=0,sticky='w')
            self.name_label = tk.Label(text=f'name:                {self.name}', name='customer name')
            self.name_label.grid(row=6, column=0, padx=10,pady=0, sticky='w')
            self.contact_label = tk.Label(text=f'contact:             {self.contact}', name='customer contact')
            self.contact_label.grid(row=7, column=0, padx=10,pady=0, sticky='w')
            self.address_label = tk.Label(text=f'address:             {self.address}', name='customer address')
            self.address_label.grid(row=8, column=0, padx=10, pady=0, sticky='w')
...
class Customers():
    ...
    def edit_frame(self):
        ...
        if len(selection) != 1:
            ...
            tk.Label(name='customer name label', text=f'name:                {self.name}').grid(row=7, column=0, padx=10, pady=0, sticky='w')
            tk.Label(name='customer contact label', text=f'contact:            {self.contact}').grid(row=8, column=0, padx=10, pady=0, sticky='w')
            tk.Label(name='customer address label', text=f'address:            {self.address}').grid(row=9, column=0, padx=10, pady=0, sticky='w')

            self.new_name = tk.Entry(name='customer name entry', width=52, foreground='grey')
            self.new_name.grid(row=7, column=1, padx=10, pady=0, sticky='w', columnspan=2)
            ...
            self.new_contact.grid(row=8, column=1, padx=10, pady=0, sticky='w', columnspan=2)
            ...
            self.new_address.grid(row=9, column=1, padx=10, pady=0, sticky='w', columnspan=2)
            ...
            self.exit_button = tk.Button(master=self, name='exit', text='Exit', width=15, command=lambda: Customers.show(self))
            self.exit_button.grid(row=10, column=2, sticky='w', padx=10, pady=10)
...
class Analysis():
    ...
    def relations(self):
        Analysis.show(self)
        canvas = analysis.relations(self)
        widget = canvas.get_tk_widget()
        widget.grid(row=1, column=0, padx=10, pady=10, columnspan=4)  # Размещаем холст в окне
class Admin(Window):
    def __init__(self):
    ...
    def importfile(self, attr, filename, deactbutton, column):
        ...
        self.csv.grid(row=1, column=column, padx=10, pady=10, sticky='n')
        self.json = tk.Button(master=self, name='json', text='json', width=5, height=1, command=None)
        self.json.grid(row=1, column=column+1, padx=10, pady=10, sticky='n')
        self.file = tk.Entry(master=self, width=15, foreground='grey')
